import sys
from math import atan, atan2, degrees, radians, sin, cos
...
class Drawable:
    ...
    def move(self):
        ...
        xpos = min(max((rect[0] + self.step[0]), 0), 800)
        ypos = min(max((rect[1] + self.step[1]), 0), 800)
...
class Ship(Drawable):
    ...
    def draw(self):
        rotated = pygame.transform.rotate(self.image, degrees(atan2(self.theta[1], self.theta[0])))
        rect = rotated.get_rect()
    ...
    def tick(self):
        # self.power *= 0.5
        self.step[0] = cos(atan2(self.theta[1], self.theta[0])) * self.power
        self.step[1] = sin(atan2(self.theta[1], self.theta[0])) * -self.power
        self.move()
        ...
        if not game_over:
            ...
            for shot in shots:
                if shot.count < shot.max_count:
                    ...
                    shot_x = shot.power * cos(atan2(ship.theta[1], ship.theta[0]))
                    shot_y = shot.power * -sin(atan2(ship.theta[1], ship.theta[0]))
